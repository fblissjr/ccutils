...
- HTML snapshots are NOT tracked in git (`git ls-files tests/__snapshots__` is empty), so they are local baselines, not shared evidence -- weigh a passing snapshot count accordingly. They need `--snapshot-update` after ANY change to `transcript.css` / `macros.html` / `base.html`; CSS classes used in templates MUST exist in `transcript.css` (Jinja2 won't warn); template variables render empty, not error -- assert on rendered output.
