...
## v0.15 (current rebuild)

### Why a rebuild

The original star schema (v0.14) string-matched against `tool_result` text for plan-revision outcome classification, even though structural signals (`is_error: true/false`, dict-vs-string `toolUseResult` shape) were already in the JSONL. An empirical scan of 1,461 local sessions made the broader gap visible:

- `toolUseResult` (entry-level structured per-tool payload): 14,674 entries carried it; v0.14 captured 0%.
- Progress entries: 53,575 emitted; v0.14 captured ~2%.
- 12 entry types in the JSONL; v0.14 modeled 5 fully.
- 7 system subtypes; v0.14 modeled 2.
- 23 attachment subtypes; v0.14 modeled 3.
- Message-level `stop_reason`, `context_management.applied_edits`, `requestId`, `apiError`, `isApiErrorMessage`: all dropped.
- Cache-arithmetic bugs in `semantic_cost_analysis`: `cache_hit_rate_pct` denominator was wrong; `cache_creation_tokens` rolled up as a single number, losing the 5m/1h split required for the 1.25x-vs-2x pricing tiers.

The rethink rebuilds the ETL and the schema together. Long-form rationale lives in `internal/plans/etl_rethink_proposal.md`; the empirical scan lives in `internal/research/claude_code_jsonl_metadata.md`.
...
DuckDB:
    ...
    fact_pr_links, fact_session_summary          (Tier 3 -- warehouse)
```
...
### Populator order (`run_v15_etl`)

The orchestrator at `src/ccutils/etl/orchestrator.py` runs per session:
...
4. Run 11 fact populators, each via the shared `lineage_upsert` helper in `src/ccutils/etl/upsert.py`:
   ...
   - `fact_pr_links`
5. `fact_session_summary` LAST (aggregates over the populated facts above).

### What's captured that v0.14 missed

- **R1 -- `toolUseResult` structured payload**: per-tool typed columns on `fact_tool_results` (Edit `structured_patch`, Bash `exit_code`/`interrupted`, Read `num_lines`/`total_lines`, Agent rollups including `total_duration_ms`), plus a JSON catch-all for unknown tools.
- **R11 -- cache-token split**: `fact_token_usage` and `fact_session_summary` split `cache_creation_tokens` into `cache_creation_5m_tokens` (1.25x pricing) and `cache_creation_1h_tokens` (2x pricing). `total_uncached_equivalent_tokens` correctly = input + creation_total + read.
- **R16 -- tri-state `is_error`**: nullable BOOLEAN on `fact_tool_results` instead of coercing missing-vs-false.
- **All 12 entry types**: `file-history-snapshot`, `queue-operation`, `pr-link`, `last-prompt`, etc. each have a fact at the appropriate grain.
- **All 7 system subtypes** on `fact_system_events`.
...
- v0.15 tests: 133 passing across parser, lineage, staging, and the 11 per-fact populators.
- Full suite: ~892 passing / ~160 failing. The failures all target legacy v0.14 fact columns that the v0.15 facts replaced (e.g. `fact_messages.actual_input_tokens` -> `fact_messages.input_tokens`). They clear when Phase D ports remaining legacy facts.
- End-to-end smoke test on a real 7,509-line session yields: fact_messages=3007, fact_tool_uses=1084, fact_tool_results=1084, fact_token_usage=1845, fact_attachments=2712, fact_system_events=114, fact_meta_events=1416, fact_file_history_snapshots=162, fact_queue_operations=98, fact_session_summary=1.

### Querying v0.15

```sql
-- Sessions ranked by uncached-equivalent token cost (R11 cache-split fix)
SELECT
  ds.session_id,
