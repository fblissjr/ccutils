...
class TestImportCommandDuckDB:
    """Tests for import DuckDB export."""

    def test_duckdb_export(self, tmp_path):
        """DuckDB export creates database file."""
        export_dir = _create_mock_export(tmp_path, num_conversations=1)
    ...
    def test_duckdb_default_path(self, tmp_path):
        """DuckDB export without -o uses default path."""
        export_dir = _create_mock_export(tmp_path, num_conversations=1)
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result = runner.invoke(
                cli,
                ["import", str(export_dir), "--format", "duckdb"],
            )

        assert result.exit_code == 0

    def test_duckdb_with_conversation_filter(self, tmp_path):
        """DuckDB export with -c filters conversations."""
        export_dir = _create_mock_export(tmp_path, num_conversations=3)
        db_path = tmp_path / "output.duckdb"
        ...
        result = runner.invoke(
            ...
            [
                ...
                str(db_path),
                "-c",
                "conv-0000-uuid-placeholder-value",
            ],
        )
