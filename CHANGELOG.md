...
- `black` from the dev dependency group -- declared with no config section, no pre-commit hook, and no invocation anywhere in the repo. `pytest`, `pytest-httpx` and `syrupy` are retained: they are imported by 53, 2 and 4 test files respectively and `uv run pytest tests/` does not run without them.
