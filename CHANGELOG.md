...
- **`semantic_etl_runs` rows_* rollups scope to `upsert:%` steps** like the run/batch derivations (the unscoped view materially overstated rows against the batch's true total). `step_count` still covers every DAG node.

### Fixed (review of the metadata layer)
- **Run/batch fact counts exclude staging rows.** `EtlRun.complete` / `BatchRun.complete` summed `rows_inserted` over ALL steps, so the `load_staging` step's staging row count inflated `facts_inserted` (and every batch `rows_*` total) by one per JSONL line. Rollups now sum `upsert:%` steps only; stage steps keep their real counts at step grain.
